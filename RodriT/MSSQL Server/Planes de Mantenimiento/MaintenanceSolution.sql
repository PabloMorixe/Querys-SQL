...
USE [SqlMant] -- Specify the database in which the objects will be created.
...
SET @BackupDirectory     = N'E:\MSSQL2k17\MSSQL14.MSSQLSERVER\MSSQL\Backup' -- Specify the backup root directory. If no directory is specified, the default backup directory is used.
SET @CleanupTime         = 19         -- Time in hours, after which backup files are deleted. If no time is specified, then no backup files are deleted.
SET @OutputFileDirectory = N'E:\MSSQL2k17\MSSQL14.MSSQLSERVER\MSSQL\Backup' -- Specify the output file directory. If no directory is specified, then the SQL Server error log directory is used.
