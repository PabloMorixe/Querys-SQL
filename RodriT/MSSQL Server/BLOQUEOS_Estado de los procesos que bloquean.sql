/*MUESTRA ESTADO DE LOS PROCESOS QUE BLOQUEAN*/
...
(
   ...
   INNER JOIN  sys.dm_exec_requests AS br
       ON                        wait.blocking_session_id = br.session_id
   INNER JOIN  sys.dm_exec_requests AS wr
       ON                        lock.request_session_id = wr.session_id
   INNER JOIN        sys.dm_tran_locks AS block
